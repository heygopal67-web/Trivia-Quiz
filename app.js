...
class TriviaQuiz {
  ...
  async startQuiz() {
    ...
    try {
      ...
      this.questions = await triviaAPI.fetchQuestions(
        category,
        difficulty,
        this.totalQuestions
      );
  ...
  hideLoadingState() {
    ...
    submitBtn.innerHTML = '<i class="fas fa-play"></i> Start Quiz';
  }
  ...
  calculateScore(isCorrect, answerTime) {
    ...
    const speedBonus = Math.floor(
      timeBonus * this.currentSettings.bonusMultiplier
    );
    score += speedBonus;
  ...
  checkHighScore() {
    ...
    if (
      topScores[0].name === this.playerName &&
      topScores[0].score === this.score
    ) {
